...
export class Services {
 ...
 async labelEntries(data){const r=await this.read('etiquetas.list',data,true);return r.entries.map((r,i)=>({...r,rowNumber:i+2,criadoEm:r.createdAt,criadoPor:r.createdBy,editadoEm:r.updatedAt,editadoPor:r.updatedBy,resumoEdicao:(r.editHistory||[]).map(x=>`${x.at} ${x.by}: ${(x.fields||[]).join(', ')}`).join('\n')}));}
