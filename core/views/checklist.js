...
(() => {
  ...
  function positionArsenalInfoBanner(){const banner=$('arsenalInfoBanner'),target=document.querySelector('.report-date-banner .quick-actions'),dialog=$('reportDialog');if(!banner||banner.hidden||!target||!dialog)return;const dialogRect=dialog.getBoundingClientRect(),targetRect=target.getBoundingClientRect();banner.style.top=Math.max(0,Math.round(targetRect.top-dialogRect.top))+'px';banner.style.left=Math.max(0,Math.round(targetRect.left-dialogRect.left))+'px';banner.style.width=Math.max(1,Math.round(targetRect.width))+'px';const detailVisible=!$('arsenalInfoDetail')?.hidden;const minimumHeight=detailVisible?150:96;const maximumHeight=Math.max(minimumHeight,Math.round(dialogRect.height*.55));banner.style.height=Math.min(Math.max(Math.round(targetRect.height),minimumHeight),maximumHeight)+'px';}
