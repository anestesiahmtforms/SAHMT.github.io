...
(() => {
  ...
  function positionArsenalInfoBanner(){const banner=$('arsenalInfoBanner'),target=document.querySelector('.report-date-banner'),dialog=$('reportDialog');if(!banner||banner.hidden||!target||!dialog)return;const dialogRect=dialog.getBoundingClientRect(),targetRect=target.getBoundingClientRect(),inset=8;const left=Math.max(inset,Math.round(targetRect.left-dialogRect.left)+inset);const right=Math.max(inset,Math.round(dialogRect.right-targetRect.right)+inset);const available=Math.max(1,Math.round(dialogRect.width-left-right));banner.style.top=Math.max(0,Math.round(targetRect.top-dialogRect.top))+'px';banner.style.left=left+'px';banner.style.width=Math.min(available,Math.max(1,Math.round(targetRect.width-2*inset)))+'px';banner.style.height=Math.max(1,Math.round(targetRect.height))+'px';}
