...
(() => {
  ...
  function showStatus(item,state){const card=[...$('equipmentList').children].find(node=>node.querySelector('[data-unit-id]')?.dataset.unitId===item.id);const banner=card?.querySelector('.status-banner');if(!banner)return;if(!banner.hidden){closeArsenalBanners();return;}closeArsenalBanners();banner.replaceChildren();const realized=!!item.record;addText(banner,'strong',realized?'Realizado':'Não Realizado');if(realized){addText(banner,'p',item.record.email || 'E-mail não disponível.').className='status-email';}else{const check=directCheckButton(item);if(check)banner.append(check);}card.classList.add('has-open-status');banner.hidden=false;}
  function showMaintenance(item){const card=[...$('equipmentList').children].find(node=>node.querySelector('[data-unit-id]')?.dataset.unitId===item.id);const banner=card?.querySelector('.status-banner');if(!banner)return;if(!banner.hidden){closeArsenalBanners();return;}closeArsenalBanners();banner.replaceChildren();addText(banner,'strong','Inativo');card.classList.add('has-open-status');banner.hidden=false;}
