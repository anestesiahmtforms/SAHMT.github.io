...
(() => {
  ...
  function renderReport(data){
    ...
    addText(statusWrap,'p',statusText).className='signature signature-'+state;
    const incompletePending=!data.signature&&isToday&&data.canSign&&(done!==activeItems.length||!done);
    const incompleteSigned=!!data.signature?.incomplete;
    if(incompletePending||incompleteSigned){const incompleteButton=addText(statusWrap,'button','Assinar sem concluir');incompleteButton.type='button';incompleteButton.className='sign-incomplete-button'+(incompleteSigned?' is-signed':'');incompleteButton.setAttribute('aria-label',incompleteSigned?'Ver justificativa da assinatura sem concluir':'Assinar relatório sem concluir todos os checklists');incompleteButton.onclick=()=>openIncompleteSignatureBanner(data.signature);}
