...
(() => {
  ...
  function positionArsenalInfoBanner(){const banner=$('arsenalInfoBanner'),grid=$('equipmentList'),dialog=$('reportDialog');if(!banner||banner.hidden||!grid||!dialog)return;const dialogRect=dialog.getBoundingClientRect(),gridRect=grid.getBoundingClientRect();banner.style.top=Math.max(0,Math.round(gridRect.top-dialogRect.top))+'px';banner.style.left=Math.max(0,Math.round(gridRect.left-dialogRect.left))+'px';banner.style.width=Math.max(1,Math.round(gridRect.width))+'px';banner.style.height=Math.max(1,Math.round(gridRect.height))+'px';}
