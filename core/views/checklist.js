...
(() => {
  ...
  let session = null, stream = null, scanning = false, cameraDetector = null, current = null, report = null, lastValidReportDay = '';
  const reportCache = new Map(), CHECKLIST_REPORT_CACHE_MS = 30000;
  ...
  let pendingRecord = null, pendingRecordMode = 'qr', pendingSignature = null, busy = false, reportSyncTimer = null, reportSyncStartedAt = 0, reportSyncPending = false;
  ...
  function startReportSync() {
    clearInterval(reportSyncTimer);reportSyncTimer=null;reportSyncStartedAt=Date.now();syncIndicator('syncing',0);
  }
  ...
  async function refreshReport(day){startReportSync();try{const data=await api('report',{day},{timeoutMs:12000,cacheTtlMs:CHECKLIST_REPORT_CACHE_MS});if(report?.day===day)renderReport(data);finishReportSync();return data;}catch{failReportSync();return null;}}
  ...
  function renderReport(data){
    ...
    if(data.signature){appendSignaturePerson($('responsible'),'Assinou',data.signature.email,'signer-email signature-signed');}
  ...
  const statusText=data.signature?'':mode==='cached'?'Mostrando dados recentes enquanto atualiza.':mode==='history'?'Histórico do dia — somente consulta.':mode==='stale'?'O checklist mudou após a assinatura. É necessária uma nova assinatura.':mode==='locked'?'Assinatura indisponível para esta conta.':'';
    ...
    if(data.signature){
      appendSignaturePerson(statusWrap,'Responsável',responsibleEmail || (data.responsible && data.responsible.reason) || 'Responsável indisponível.','responsible-email signature-'+state);
      appendSignaturePerson(statusWrap,'Assinou',data.signature.email,'signer-email signature-signed');
      const resultButton=addText(statusWrap,'button',data.signature.incomplete?'Assinado sem concluir':'Assinado após concluído');
      resultButton.type='button';resultButton.disabled=true;resultButton.className='signature-result-button '+(data.signature.incomplete?'incomplete':'complete');
      const signedAt=new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(data.signature.at)).replace(',','');
      addText(statusWrap,'span',`Assinado em ${signedAt}`).className='signature-date';
      if(data.signature.justification){
        const justification=document.createElement('section');justification.className='incomplete-justification-display';
        addText(justification,'strong',data.signature.incomplete?'JUSTIFICATIVA DE ASSINATURA SEM CONCLUIR':'JUSTIFICATIVA DA ASSINATURA');
        addText(justification,'p',String(data.signature.justification));statusWrap.append(justification);
      }
    }
  ...
  function openReportDialog(){const dialog=$('reportDialog');const title=dialog?.querySelector('.report-title h2');if(title)title.textContent='RELATÓRIO DIÁRIO - CHECKLIST';dialog.showModal();dialog.focus({preventScroll:true});}
  let reportRequest = 0;
  async function loadReport(){
    ...
    const refreshInBackground=async()=>{
      try{
        const data=await api('report',{day},{timeoutMs:12000,cacheTtlMs:CHECKLIST_REPORT_CACHE_MS});
  ...
  $('confirmIncompleteSignature').onclick=()=>run(async()=>{
    ...
    try{const result=await api('sign',{day:report.day,revision:report.revision,accepted:true,signWithoutComplete:true,justification,requestId:pendingSignature});rememberReport(result);renderReport(result);pendingSignature=null;}catch(error){await loadReport().catch(()=>{});throw error;}finally{button.disabled=false;}
