...
(() => {
  ...
  const statusText=data.signature&&!data.signature.incomplete?signatureLabel(state,data.signature)+' — '+(data.signature.name || data.signature.email)+' ('+data.signature.email+'), em '+new Date(data.signature.at).toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo'}).replaceAll('/','-')+'.':data.signature?.incomplete?'':mode==='history'?'Histórico do dia — somente consulta.':mode==='stale'?'O checklist mudou após a assinatura. É necessária uma nova assinatura.':mode==='locked'?'Assinatura indisponível para esta conta.':'';
    ...
    closeIncompleteSignatureBanner();
  }  function showStatus(item,state){const card=[...$('equipmentList').children].find(node=>node.querySelector('[data-unit-id]')?.dataset.unitId===item.id);const banner=card?.querySelector('.status-banner');if(!banner)return;if(!banner.hidden){banner.hidden=true;return;}document.querySelectorAll('.equipment .status-banner').forEach(node=>{node.hidden=true;});banner.replaceChildren();const title=state==='SIM'?'Checklist Realizado!':state==='NAO'?'Alerta!':'Checklist não realizado!';addText(banner,'strong',title);if(state==='NAO'&&item.record?.occurrence)addText(banner,'p',item.record.occurrence);if(item.record){addText(banner,'p',`Registrado por: ${item.record.email}`).className='status-email';}banner.hidden=false;}
  function showMaintenance(item){const card=[...$('equipmentList').children].find(node=>node.querySelector('[data-unit-id]')?.dataset.unitId===item.id);const banner=card?.querySelector('.status-banner');if(!banner)return;if(!banner.hidden){banner.hidden=true;return;}document.querySelectorAll('.equipment .status-banner').forEach(node=>{node.hidden=true;});banner.replaceChildren();addText(banner,'strong','Em manutenção');addText(banner,'p','Checklist temporariamente inativo para este arsenal.');banner.hidden=false;}
  ...
  if($('today'))$('today').textContent=new Intl.DateTimeFormat('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',timeZone:'America/Sao_Paulo'}).format(new Date()).replaceAll('/','-');
  lastValidReportDay=dateKey();$('reportDate').value=lastValidReportDay;
