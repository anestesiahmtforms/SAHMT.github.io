...
(() => {
  ...
  function renderReport(data){
    ...
    if(data.signature){appendEmail($('responsible'),'ASSINADO POR',data.signature.email,'signer-email signature-signed');addText($('responsible'),'span',new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(data.signature.at)).replace(',','')).className='signature-date';}
  ...
  const statusText=data.signature&&!data.signature.incomplete?signatureLabel(state,data.signature)+' — '+(data.signature.name || data.signature.email)+' ('+data.signature.email+'), em '+new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(data.signature.at)).replace(',','')+'.'+(data.signature.justification?' Justificativa: '+data.signature.justification:''):data.signature?.incomplete?'':mode==='history'?'Histórico do dia — somente consulta.':mode==='stale'?'O checklist mudou após a assinatura. É necessária uma nova assinatura.':mode==='locked'?'Assinatura indisponível para esta conta.':'';
    ...
    if(incompleteSigned){const justification=document.createElement('section');justification.className='incomplete-justification-display';addText(justification,'strong','ASSINADO SEM CONCLUIR');addText(justification,'p',String(data.signature.justification || 'Justificativa não informada.'));statusWrap.append(justification);}
  ...
  function fitStatusBanner(card,banner){const button=card?.querySelector('[data-unit-id]');if(!button)return;const width=Math.max(1,Math.round(button.getBoundingClientRect().width));banner.style.setProperty('--status-banner-width',`${width}px`);}
