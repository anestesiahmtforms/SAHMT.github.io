...
(() => {
  ...
  const statusText=data.signature&&!data.signature.incomplete?signatureLabel(state,data.signature)+' — '+(data.signature.name || data.signature.email)+' ('+data.signature.email+'), em '+new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(data.signature.at)).replace(',','')+'.':data.signature?.incomplete?'':mode==='history'?'Histórico do dia — somente consulta.':mode==='stale'?'O checklist mudou após a assinatura. É necessária uma nova assinatura.':mode==='locked'?'Assinatura indisponível para esta conta.':'';
